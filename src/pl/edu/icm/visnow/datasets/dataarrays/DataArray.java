 ...
 * @author Bartosz Borucki, University of Warsaw, ICM The base class for all
 * data arrays used in Field objects Holds data array by itself and additional
 * information necessary for proper interpretation of data
 */
abstract public class DataArray implements Serializable, Hashable {
    ...
    public DataArray(DataArraySchema schema, int ndata) {
        ...
        if (ndata <= 0) {
            throw new IllegalArgumentException("ndata has to be positive.");
        }
        this.schema = schema;
        this.ndata = ndata;
    ...
    public DataArray(int type, int ndata, int veclen) {
        this(type, ndata, veclen, "Data", "", null);
    }
    ...
    public static DataArray create(boolean[] data, int veclen, String name, String units, String[] userData) {
        return new BooleanDataArray(data, veclen, name, units, userData);
    }
     ...
     * @param symmetric symmetric array indicator checks for partameter
     * compatibility
     */
    ...
    /**
    * Return true if data array is of numeric type, but not including complex type:
   ...
   /**
    * Return true if data array is of any numeric type:
   ...
   /**
    * Return true if data is of numeric type or can be converted to numeric type (using get?Data methods):
    * byte, short, int ,float, double, complex, boolean, logic.
    * 
    * This does not include string type and object type (which BTW contains no-empty get*Data methods).
    */
   public boolean isNumericConvertible()
   {
       //TODO: this should be probably changed to abstract and implemented in inherited classes (or opposite - if one considers it as a schema property than moved into DataArraySchema)
      return schema.isNumeric() || schema.getType() == DataArray.FIELD_DATA_BOOLEAN || schema.getType() == DataArray.FIELD_DATA_LOGIC;
   }
    ...
    /* scales new physical min max on the basis of old min/max nad new min/max*/
    protected void recomputePhysMinMax(float newMinv, float newMaxv) {
        float oldMinv = getMinv();
        float oldMaxv = getMaxv();        
        float oldPhysMinv = getPhysMin();
        float oldPhysMaxv = getPhysMax();
        if(oldPhysMaxv == oldMaxv && oldPhysMinv == oldMinv) {
            setPhysMin(newMinv);
            setPhysMax(newMaxv);
            return;            
        }
        if(oldMaxv == oldMinv) {
            float s = (oldPhysMaxv)/(oldMaxv);
            float newPhysMinv = s*newMinv;
            float newPhysMaxv = s*newMaxv;
            setPhysMin(newPhysMinv);
            setPhysMax(newPhysMaxv);            
            return;
        }
        
        float s = (oldPhysMaxv-oldPhysMinv)/(oldMaxv-oldMinv);
        //pv = s*(v-oldMinv) + oldPhysMinv;
        float newPhysMinv = s*(newMinv-oldMinv) + oldPhysMinv;
        float newPhysMaxv = s*(newMaxv-oldMinv) + oldPhysMinv;
        setPhysMin(newPhysMinv);
        setPhysMax(newPhysMaxv);
    }
    ...
    public DataArray concatenate(ArrayList<DataArray> concatenatedArrays) {
        int n = getNData();
...
//         n += cD.getLenth();
     ...
     * @return - true if name, type, veclen, units and time vectors of s are
     * compatible
     */
