...
public class IrregularField extends Field {
    ...
    public String shortDescription() {
        ...
        s.append("<br>").append(data.size()).append(" components");
        s.append("</html>");
        return s.toString();
    }
    ...
    public IrregularField clone() {
        IrregularField clone = new IrregularField(nNodes);
    ...
    public IrregularField cloneCoordsDeep() { // TODO: should be removed after cloneDeep is working!!!
        IrregularField clone = new IrregularField(nNodes);
        clone.setNSpace(this.nSpace);
        clone.setCoords((TimeData<float[]>) this.timeCoords.clone());
        for (CellSet cellSet : cellSets) {
    ...
    public IrregularField flattenCellSets() {
        ...
        out.cellSets = new ArrayList<CellSet>();
    ...
    public boolean isStructureCompatibleWith(Field f) {
        if (!(f instanceof IrregularField) || f.getNNodes() != nNodes) {
            return false;
        }
        ...
        for (int i = 0; i < cellSets.size(); i++) {
            if (!cellSets.get(i).isStructCompatible(irf.getCellSet(i))) {
                return false;
            }
        }
        return true;
    ...
    public boolean isDataCompatibleWith(Field f) {
        ...
        for (int i = 0; i < cellSets.size(); i++) {
            if (!cellSets.get(i).isDataCompatibleWith(irf.getCellSet(i))) {
                return false;
            }
        }
        return true;
    ...
    public void createGeoTree() {
        ...
        int maxThr = pl.edu.icm.visnow.system.main.VisNow.availableProcessors() - 1;
        if (maxThr < 1) {
            maxThr = 1;
        }
    ...
    public IrregularField triangulate() {
        ...
        for (CellSet cs : cellSets) {
            ...
            trCs.generateDisplayData(timeCoords.getData(currentTime));
