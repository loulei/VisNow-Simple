   ...
   public static class TubeTemplate extends Glyph
   {
      public TubeTemplate(Integer lod)
      {
         ...
         for (int i=0;i<n+1;i++)
         {
            ...
            verts[6*i+1]  = verts[6*i+3+1]  = (float)(Math.sin(phi));
            verts[6*i+2]  = 0;
            verts[6*i+3+2]  = 1;
            normals[6*i  ]  = normals[6*i+3]  = (float)(Math.cos(phi));
   ...
   public static class ConeTemplate extends Glyph
   {
      ...
      public ConeTemplate(Integer lod)
      {
         ...
         for (int i=0;i<4*lod+1;i++)
         {
            ...
            normals[3*(i+1)]   = (float)(Math.cos(phi));
            normals[3*(i+1)+1] = (float)(Math.sin(phi));
   ...
   public static class Arrow3dTemplate extends Glyph
   {
      ...
      public Arrow3dTemplate(Integer lod)
      {
         ...
         for (int i = 0; i < n + 1; i++) {
            ...
            verts[6 * i + 1] = verts[6 * i + 4] = (float) (.06 * Math.sin(phi));
            verts[6 * i + 2] = 0;
            verts[6 * i + 5] = .8f;
            normals[6 * i]     = normals[6 * i + 3] = -(float) (Math.cos(phi));
            normals[6 * i + 1] = normals[6 * i + 4] = -(float) (Math.sin(phi));
            ...
            verts[m + 6 * i + 5] = 1f;
            normals[m + 6 * i]     = normals[m + 6 * i + 3] = -(float) (Math.cos(phi));
            normals[m + 6 * i + 1] = normals[m + 6 * i + 4]  = -(float) (Math.sin(phi));
