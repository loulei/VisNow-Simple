...
public class ColorMapper
{
   ...
   {
      if (colors == null || mask == null || colors.length != 4 * (end - start))
         return;
   ...
   {
      if (colors == null || colors.length != 4 * (end - start))
         colors = new byte[4 * (end - start)];
      return map(data, dataMappingParams, start, end, 0, col, colors);
   }
   ...
   public static byte[] mapTransparency(DataContainer data, TransparencyParams params, int start, int end, byte[] colors)
   {
      if (colors == null || colors.length != 4 * (end - start))
         colors = new byte[4 * (end - start)];
      return mapTransparency(data, params, start, end, 0, colors);
   }
