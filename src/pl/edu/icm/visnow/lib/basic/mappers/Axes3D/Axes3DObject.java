...
public class Axes3DObject extends DataMappedGeometryObject {
    ...
    public void update(Field field, Axes3DParams params) {
        ...
        ext = field.getExtents();
        this.setExtents(field.getExtents());
        ...
        if (fontParams.isThreeDimensional()) {
            ...
            if (drawX) {
                ...
                try {
                    ...
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[0] + "<br>" + "Dropped to default one: " + defAxFormats[0], Level.WARNING));
                    for (i = 1, m = 0; i < nx; i++, m++) {
                        axes.addChild(TextBillboard.createBillboard(String.format(defAxFormats[0], rx.getLow() + i * sx), fontParams, red, 1,
                                new float[]{x0 + i * gx, t[1] - .3f * gy, t[2] - .3f * gz}, Billboard.ROTATE_ABOUT_POINT, bSphere));
                    }
                }
            }
            if (drawY) {
                ...
                try {
                    ...
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[1] + "<br>" + "Dropped to default one: " + defAxFormats[1], Level.WARNING));
                    for (i = 1; i < ny; i++, m++) {
                        axes.addChild(TextBillboard.createBillboard(String.format(defAxFormats[1], ry.getLow() + i * sy), fontParams, green, 1,
                                new float[]{t[6] - .3f * gx, y0 + i * gy, t[8] - .3f * gz}, Billboard.ROTATE_ABOUT_POINT, bSphere));
                    }
                }
            }
            if (drawZ) {
                ...
                try {
                    ...
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[2] + "<br>" + "Dropped to default one: " + defAxFormats[2], Level.WARNING));
                    for (i = 1; i < nz; i++, m++) {
                        axes.addChild(TextBillboard.createBillboard(String.format(defAxFormats[2], rz.getLow() + i * sz), fontParams, blue, 1,
                                new float[]{t[12] - .3f * gx, t[13] - .3f * gy, z0 + i * gz}, Billboard.ROTATE_ABOUT_POINT, bSphere));
                    }
                }
            }
            ...
            int iAx = 0;
            if (drawX) {
                axisNames[iAx] = new CXYZString(ad[0], new Color(val1, val0, val0), x1 + .3f * gx, t[1], t[2], font, 1.5f * fontParams.getSize());
                iAx += 1;
            }
            if (drawY) {
                axisNames[iAx] = new CXYZString(ad[1], new Color(val0, val1, val0), t[6], y1 + .3f * gy, t[8], font, 1.5f * fontParams.getSize());
                iAx += 1;
            }
            if (drawZ) {
                axisNames[iAx] = new CXYZString(ad[2], new Color(val0, val0, val1), t[12], t[13], z1 + .3f * gz, font, 1.5f * fontParams.getSize());
                iAx += 1;
            }
                ...
                try {
                    for (i = 1; i < nx; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(axFormats[0], rx.getLow() + i * sx),
                                ...
                                x0 + i * gx, t[1] - .3f * gy, t[2] - .3f * gz, font, fontParams.getSize());
                    }
                } catch (IllegalFormatException ex) {
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[0] + "<br>" + "Dropped to default one: " + defAxFormats[0], Level.WARNING));
                    for (i = 1; i < nx; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(defAxFormats[0], rx.getLow() + i * sx),
                                new Color(val1, val0, val0),
                                x0 + i * gx, t[1] - .3f * gy, t[2] - .3f * gz, font, fontParams.getSize());
                    }
                }
                ...
                try {
                    for (i = 1; i < ny; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(axFormats[1], ry.getLow() + i * sy),
                                ...
                                t[6] - .3f * gx, y0 + i * gy, t[8] - .3f * gz, font, fontParams.getSize());
                    }
                } catch (IllegalFormatException ex) {
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[1] + "<br>" + "Dropped to default one: " + defAxFormats[1], Level.WARNING));
                    for (i = 1; i < ny; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(defAxFormats[1], ry.getLow() + i * sy),
                                new Color(val0, val1, val0),
                                t[6] - .3f * gx, y0 + i * gy, t[8] - .3f * gz, font, fontParams.getSize());
                    }
                }
                ...
                try {
                    for (i = 1; i < nz; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(axFormats[2], rz.getLow() + i * sz),
                                ...
                                t[12] - .3f * gx, t[13] - .3f * gy, z0 + i * gz, font, fontParams.getSize());
                    }
                } catch (IllegalFormatException ex) {
                    VisNow.get().userMessageSend(new UserMessage("", "axes 3D", "Incorrect text format", "Specified format is incorrect: " + axFormats[2] + "<br>" + "Dropped to default one: " + defAxFormats[2], Level.WARNING));
                    for (i = 1; i < nz; i++, m++) {
                        axisLegends[m] = new CXYZString(String.format(defAxFormats[2], rz.getLow() + i * sz),
                                new Color(val0, val0, val1),
                                t[12] - .3f * gx, t[13] - .3f * gy, z0 + i * gz, font, fontParams.getSize());
                    }
                }
        ...
        if (axisLegends != null) {
            for (int i = 0; i < axisLegends.length; i++) {
                if (axisLegends[i] != null) {
                    ...
                    axisLegends[i].draw(vGraphics, w, h);
                }
            }
        }
        if (axisNames != null) {
            for (int i = 0; i < axisNames.length; i++) {
                if (axisNames[i] != null) {
                    ...
                    axisNames[i].draw(vGraphics, w, h);
                }
            }
        }
    }
