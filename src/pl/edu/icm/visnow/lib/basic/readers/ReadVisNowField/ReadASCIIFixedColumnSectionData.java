    ...
    public static int readSectionData(SectionModel model, LineNumberReader reader, String filePath, String decimalSep) {
        ...
        try {
                ...
                for (int l = 0; l < nComps; l++) {
                    ...
                    switch (types[l]) {
                        case DataArray.FIELD_DATA_BOOLEAN:
                            boolArrs[l][ind[l]] = item.startsWith("1") || item.startsWith("t");
                            ind[l] += vlens[l];
                            break;
                        ...
                        case DataArray.FIELD_DATA_FLOAT:
                            ...
                            floatArrs[l][ind[l]] = Float.parseFloat(tmp);
                            ind[l] += vlens[l];
                            break;
                        case DataArray.FIELD_DATA_DOUBLE:
                            ...
                            dblArrs[l][ind[l]] = Double.parseDouble(tmp);
                            ind[l] += vlens[l];
                            break;
                    }
                }
            ...
            for (int l = 0; l < nComps; l++) {
                switch (types[l]) {
                    case DataArray.FIELD_DATA_FLOAT:
                        if (!FloatingPointUtils.isFinite(floatArrs[l])) {
                            throw new IOException("dataset contains NaN, Float.POSITIVE_INFINITY or Float.NEGATIVE_INFINITY elements");
                        }
                        break;
                    case DataArray.FIELD_DATA_DOUBLE:
                        if (!FloatingPointUtils.isFinite(dblArrs[l])) {
                            throw new IOException("dataset contains NaN, Float.POSITIVE_INFINITY or Float.NEGATIVE_INFINITY elements");
                        }
                        break;
                }
