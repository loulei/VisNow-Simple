 ...
 *                            results: eigenvectors, iterations) */
   ...
   for (kr=0;kr<ir;kr++)
      ...
      for (i=1;i<n;i++)
         ...
         if (Math.abs(d[i])<=dm) dm=Math.abs(d[i]);
         }
      if (dm+bm==dm || bm==0.) return 2;
      diff=d[q]-d[p];
      g=100.*bm;
               ...
               for (int k = 0; k < dims[0]; k++, m++)
               {
                  ...
                  if (invert(jacobi, invJacobi))
                  {
                     for (int l = 0; l < 3; l++)
                     {
                        v[l] = 0;
                        for (int n = 0; n < 3; n++)
                           v[l] += invJacobi[l][n] * vec[3 * m + n];
                     }
                     System.arraycopy(v, 0, pullVec, 3*m, 3);
                  }
               }
      }
      else
